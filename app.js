...
function extractForecastData(jsonData) {
    ...
    for (let i = 0; i < 7; i++) {
        ...
        forecast[i] = {
            date: jsonData.forecast.forecastday[i].date
        ...
        for (let j = 0; j < 24; j++) {
            forecast[i].hour[j] = {
                time: jsonData.forecast.forecastday[i].hour[j].time,
...
function start() {
    apiCall(`http://api.weatherapi.com/v1/current.json?key=1b99b3dfb10948888fe125556233007&q=New+Delhi`);
    forecastAPICall(`http://api.weatherapi.com/v1/forecast.json?key=1b99b3dfb10948888fe125556233007&q=New+Delhi&days=7`);
